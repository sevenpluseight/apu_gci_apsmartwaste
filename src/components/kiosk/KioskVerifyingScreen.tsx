...
interface KioskVerifyingScreenProps {
  title: string;
  subtitle: string;
  steps: Step[];
  currentStep: number;
...
export default function KioskVerifyingScreen({
  title,
  subtitle,
  steps,
  currentStep,
}: KioskVerifyingScreenProps) {
  const progress = ((currentStep + 1) / steps.length) * 100;
  const ActiveIcon = steps[currentStep].icon;

  return (
    <div className="flex h-full flex-col items-center justify-center text-center">
      <h2 className="text-4xl font-bold text-[#0BCB51]">{title}</h2>

      <p className="mt-3 text-base text-[#B2B2B2]">{subtitle}</p>
      ...
      <div className="mt-10 flex h-40 w-40 items-center justify-center rounded-full border border-[#0BCB51]/30 bg-[#0BCB51]/10 shadow-[0_0_45px_rgba(11,203,81,0.25)]">
        <ActiveIcon size={64} className="animate-pulse text-[#0BCB51]" />
      </div>
                ...
                <div key={step.label} className="flex flex-col items-center">
                  ...
                  <p
                    className={`mt-3 text-xs font-medium transition-colors duration-500 ${
                      isActive || isDone ? "text-white" : "text-[#6F6F6F]"
                    }`}
                  >
                    {step.label}
                  </p>
