...
const navItems = [
  { label: "Overview", icon: LayoutDashboard, href: "/facility" },
    ...
    <header className="h-20 w-full border-b border-[#2A2A2A] bg-[#191919]">
      <div className="mx-auto flex h-full max-w-360 items-center justify-between px-10">
        <div className="flex items-center gap-10">
          <Link 
            href="/"
            className="flex items-center gap-1.5 text-sm text-[#B2B2B2] transition-colors duration-300 hover:text-white"
          >
            <ArrowLeft size={18} />
            APSpace
          </Link>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`relative flex h-full min-w-16 flex-col items-center justify-center gap-1 text-sm transition-colors duration-300 ${
                  isActive
                    ? "text-[#0BCB51]"
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId="facility-nav-indicator"
